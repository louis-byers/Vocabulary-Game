...
export function StudentApp({ title }: { title: string }) {
  ...
  function next_word() { 
    const word_obj = word_list.words[Math.floor(Math.random() * word_list.words.length)]; 
    hint = word_obj.hint;
    word = word_obj.word;
  ...
  function get_word_list_items() {
    const listItems = data.word_lists.map((word_list: WordList) => {
      return <li onClick={((e) => handleSetWordList(e, word_list.id))} id={word_list.id}><b>{word_list.name}:</b> word_list.words.length</li>
    });
    wordLists = {listItems};
  }
  ...
  function get_words_from_game() {
    const listItems = words_in_game.map((word: Word) => {
      return <li  id='word${word.word}'><b>word.word:</b> word.hint</li>
    });
    return {listItems};
  }
 ...
 function handleToGameWords(e: any) {
  ...
  async function handleSetWordList(e: React.MouseEvent<HTMLLIElement, MouseEvent>, id: string) {
    try {
  ...
  function handleCourseChange(e: { target: { value: React.SetStateAction<string>; }; }) {
  ...
  return (
    <>
      <div id="splash-screen" className={screen === 'splash' ? "content" : "hidden-content"}>
        <h1>Vocab Game</h1>
        <div>Splash Screen</div>
        <div>tap screen to start</div>
        <button onClick={handleToCourse} id="start-btn" disabled={
                false
              }>  
            Start
          </button>
      ...
      <div id="course-selection" className={screen === 'course' ? 'content' : 'hidden-content'}>
        <h1>Vocab Game</h1>
        <div id="course-select">
          <form id="course-select-form"  onSubmit={handleSubmitCourse}>
            {error === 'error-course' ? <span id='errorCourse' className='error'>Failed to load course guid.</span> : null}
            <label>Enter Course Code: </label>
            <input type="text" id="course-guid" name="course-guid" placeholder="qwerty" value={course_code} onChange={handleCourseChange}/><br></br>
            <button type="submit" id="submit-course-code"  
      ...
      <div id="word-list-selection" className={screen === 'word-list' ? "content"  : "hidden-content"}>
        <h1>Vocab Game</h1>
        {error === 'error-course' ? <span id='errorCourse' className='error'>Failed to load course guid.</span> : null}
        <div id="word-list-select">
          <form id="course-select-form"  onSubmit={handleSubmitCourse}>
            {error === 'error-word-list' ? <span id='errorCourse' className='error'>Failed to load course guid.</span> : null}
      ...
      <div id="game-screen" className={screen === 'game' ? "content"  : "hidden-content"} >
      <form id="course-select-form" onSubmit={handleSubmitWord}>
      <h1 id="word-hint-header">{hint}</h1>
        <label>Enter Word: </label>
        <input type="text" id="word-test" name="word-test" placeholder="word"/><br></br>
        <button disabled={
            false
          }>
          Check
        </button>
        <div id="word-answer"><h2 id="word-answer-header" className={answered ?  is_correct? "correct-word": "incorrect-word" : "word"}>{word}</h2></div>
        <div id="score">Score: {score}</div>
        <div id="time-left">{time_remaining} {time_remaining === 1 ? 'second' : 'seconds'}</div>
      </form>
      </div>

      
      <div id="game-screen" className={screen === 'score' ? "content"  : "hidden-content"} >
      <h1 id="score-header">Congrats!</h1>
      <div>You got {score} of {words_in_game.length}!</div>
          <button id="back-to-course-btn" onClick={handleResetGame}>Replay</button>
          <button id="game-words-btn" onClick={handleToGameWords}>Review Words</button><br></br>

          <button id="back-to-word-list-btn" onClick={handleBackToWordLists}>New Wordlist</button>
          <button id="back-to-course-btn" onClick={handleBackToCourse}>Change Course</button>
      </div>

      
      <div id="game-screen" className={screen === 'game-words' ? "content"  : "hidden-content"} >
        <h1>Vocab Game</h1>
        <ul>
          <li><b>Word</b>: <i>hint</i></li>
          {}
        </ul>
